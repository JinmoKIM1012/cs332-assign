...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    List() :: (for {
      (char, frequency) <- occurrences
      i <- 1 to frequency
      tail <- combinations(occurrences.filter(pair => pair._1 > char))
    } yield List((char, i)) ::: tail)
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    y.foldLeft(x.toMap) {
      case (map, (char, frequency)) =>
        if (map(char) - frequency == 0) map - char
        else map + (char -> (map(char) - frequency))
    }.toList.sorted
  }
